    ...
    return (
        <header className="py-3 md:py-6">
            <Section className="flex flex-wrap gap-3 md:gap-6">
                <Item href="/user/profile">{t("My account")}</Item>
                <Item href="/user/profile/resume">{t("My resume")}</Item>
                <Item href="/user/profile/questionnaire">
                    {t("Questionnaire")}
                </Item>
            </Section>
        </header>
...
function Item({ href, children }: { href: string; children: React.ReactNode }) {
    return (
        <NavLink
            to={href}
            className={cn(
                "inline-flex items-center gap-1 md:gap-2 border p-1.5 md:p-3 rounded-md text-sm md:text-base"
            )}
        >
